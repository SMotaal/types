...
class DarkModeController {
    ...
    constructor(container, options) {
        ...
        if (!container || !ownerDocument) {
            if (typeof window === 'object' && window.self === window) {
                ...
                return Object.preventExtensions(Object.defineProperties(this, Object.getOwnPropertyDescriptors(Object.freeze({ ...new.target.prototype }))));
            }
        }
        let { 
        // Those options will likely change
        localStorage, scope, id, longPressTimeout, } = /** @type {DarkModeController.Options} */ ({ ...options });
        id = (id && new.target.matchID(id)) || '';
        scope =
            typeof URL === 'function'
                ? new URL(scope || './', (ownerDocument && ownerDocument.location && ownerDocument.location.href) || 'http://0.0.0.0/').pathname
        ...
        if (localStorage) {
            localStorage[key] === 'enabled'
                ...
                : localStorage[key] === 'disabled'
                    ...
                    : this.toggle(!mediaQueries ||
                        (mediaQueries.prefersDarkMode && mediaQueries.prefersDarkMode.matches) === true ||
                        (mediaQueries.prefersLightMode && mediaQueries.prefersLightMode.matches) !== true, !!(localStorage[key] = this.state = 'auto'));
            if (mediaQueries) {
        ...
        const handlerState = this &&
            ...
            (this.handleEvent[Symbol.for('dark-mode.toggler.handler.state')] ||
                (this.container &&
                    ...
                    (this.container.ownerDocument.addEventListener('onmouseup' in this.container ? 'mouseup' : 'pointerup', this.handleEvent, true),
                        (this.handleEvent[Symbol.for('dark-mode.toggler.handler.state')] = {
        ...
        switch (trigger) {
            ...
            case 'pointerdown':
                if (!target || !ownerDocument) {
                    ...
                    // target.classList.add('pointer-capture');
                    handlerState.action = 'capture';
                    handlerState.target = target;
                    handlerState.committed = false;
                    handlerState.timeout = /** @type {Window['setTimeout']} */ (setTimeout)(this.handleEvent, this.longPressTimeout);
                }
                break;
            default:
        ...
        try {
            ...
            queries != null ||
                ...
                (owner.defaultView.document !== owner
                    ...
                    : cache.set(owner, (queries = Object.freeze(Object.setPrototypeOf({
    ...
    static get createMatcher() {
        ...
        const value = Function.bind.bind(class Matcher extends RegExp {
    ...
    static get matchTrigger() {
        ...
        const value = DarkModeController.createMatcher(/(^mouseup$|^mousedown$|^pointerup$|^pointerdown$|\blight\b|\bdark\b)|$/i);

  ...
  /** @type {new (source, flags?: string) => RegExp & {(string):boolean}} */ (class Pattern extends RegExp {
    ...
    static get ownProperties() {
      ...
      const [, ...properties] = new Set([
        ...
        ...(this === Pattern
          ? [
              ...Object.getOwnPropertyNames(super.prototype),
              ...Object.getOwnPropertySymbols(super.prototype)
            ]
          : [])
      ]);
    ...
    static get ownPropertyDescriptors() {
      if (this !== Pattern)
        return Object.getOwnPropertyDescriptors(this.prototype);
      ...
      for (const property of ownProperties) {
        ...
        const value =
          !isComputed || "get" in descriptor
            ? Super.prototype[property]
            : void inheritedUndefineds.push(property);
    ...
    static bind(instance, pattern) {
      ...
      const bound =
        /** @type {WeakSet<Pattern|{}>} */ (Pattern.bind["bound"] ||
        (Pattern.bind["bound"] = new WeakSet()));
      if (bound.has(instance))
        throw TypeError(
          `Pattern.bind invoked with an instance that was previously bound`
        );
      if (!bound.has(pattern)) {
        if (!(pattern && pattern instanceof Pattern))
          throw TypeError(`Pattern.bind invoked with an invalid pattern`);
        Object.defineProperties(pattern, this.ownPropertyDescriptors);
        bound.add(pattern);
      }
      for (const property of this.ownProperties) {
        if (typeof pattern[property] === "function")
          instance[property] = pattern[property];
      }
      bound.add(instance);
      return instance;
    }
    ...
    constructor(source, flags) {
      //@ts-ignore
      super(...arguments);
      return /** @type {{(string): boolean} & RegExp} */ (new.target.bind(
        super.test.bind(this),
        this
      ));
    }
  });
...
const args = (stats.args = /** @type {string[] & {[name:string]: string}} */ ([
  ...argn
]));
...
const glob = (stats.glob = (globIndex =>
  (args.glob =
    globIndex > -1
      ? args.splice(globIndex, 1)[0]
      : Glob.test(env.GLOB)
      ? env.GLOB
      : undefined))(args.findIndex(Glob)));
  ...
  try {
    ...
    ({
      ...
      (shell.arguments = [
        ...
        // "silent",
        // "--parser",
        // require.resolve("@smotaal.io/prettier.config/parser"),
        "-l",
        `"${glob}"`
      ]),
    ...
    shell.result.stdout =
      (shell.result.stdout && shell.result.stdout.toString()) || "";
    shell.result.stderr =
      (shell.result.stderr && shell.result.stderr.toString()) || "";

    if (shell.result.stderr && !shell.result.stdout)
      throw Error(shell.result.stderr);
